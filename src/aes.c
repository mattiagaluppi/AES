#include "../include/aes.h"
#include <string.h>

/**
 * AES S-box lookup table.
 * Used for SubBytes transformations.
 * (non linear substitution)
 */
static const uint8_t sbox[256] = {
  
};

/**
 * Round constants array used in key espansion.
 * Each round constant is used to eliminate symmetries in the key schedule.
 */
static const uint8_t Rcon[11] = {
 0x00,0x01,0x02,0x04,0x08,0x10,0x20,0x40,0x80,0x1B,0x36
};

/**
 * Performs Galois Field multiplication by 2 (xtime operation).
 * Used in MixColumns transformations.
 *
 * @param x Input byte
 * @return Result of GF(2^8) multiplication by 2
 */
static inline uint8_t xtime(uint8_t x) {
    return (uint8_t)((x << 1) ^ ((x & 0x80) ? 0x1B : 0x00));
}

/**
 * Applies S-box substitution to each byte in the state matrix.
 * Part of the SubBytes transformation.
 *
 * @param state 4x4 state array
 */
static void sub_bytes(uint8_t state[4][4]){
    for (int r = 0; r < 4; ++r)
        for (int c = 0; c < 4; ++c)
            state[r][c] = sbox[state[r][c]];
}

/**
 * Cyclically shifts the rows of the state matrix.
 * - Row 1 is shifted 1 position to the left
 * - Row 2 is shifted 2 positions to the left
 * - Row 3 is shifted 3 positions to the left
 * 
 * @param state 4x4 state array
 */
static void shift_rows(uint8_t state[4][4]) {
    uint8_t tmp[4];
    for (int r = 1; r < 4; r++){
        for (int c = 0; c < 4; c++)
            tmp[c] = state[r][(c + r) % 4];
        for (int c = 0; c < 4; c++)
            state[r][c] = tmp[c];
    }
}

/**
 * Mixes the columns of the state matrix using matrix multiplication
 * in GF(2^8). This provides diffusion in the cipher.
 *
 * @param state 4x4 state array
 */
static void mix_columns(uint8_t state[4][4]){
    for (int c = 0; c < 4; ++c) {
        uint8_t a0 = state[0][c];
        uint8_t a1 = state[1][c];
        uint8_t a2 = state[2][c];
        uint8_t a3 = state[3][c];
        
        uint8_t t = a0 ^ a1 ^ a2 ^ a3;

        state[0][c] ^= t ^ xtime(a0 ^ a1);
        state[1][c] ^= t ^ xtime(a1 ^ a2);
        state[2][c] ^= t ^ xtime(a2 ^ a3);
        state[3][c] ^= t ^ xtime(a3 ^ a0);
    }
}

